...
**④ 스트리밍과 델타(delta).** 모델은 응답을 한 번에 완성해 주지 않고, 만들어지는 대로 **조각(delta)**을 흘려보냅니다. "맑고 " → "따뜻해요." 처럼요. 우리는 이 조각들을 순서대로 이어붙여 완성 문장을 만듭니다. (빠르게 보여주기 위한 방식입니다. -  (FastRTC에 emit이 이벤트 루프로 완성문장 만든다)
